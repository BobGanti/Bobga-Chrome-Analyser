...
class QuerybyDate(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.options_mm = ttk.OptionMenu(frame1, self.mm, 'select month', 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun',
                                         'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec')
        self.options_mm.pack(side='left', fill=X, padx=20)

        self.yy = StringVar(frame1)
        self.yy.set('select year')
        self.options_yy = ttk.OptionMenu(frame1, self.yy, 'select year', 2020, 2019, 2018, 2017, 2016, 2017, 2016, 2015, 2014,
                                         2013, 2012, 2011, 2010)
    ...
    def get_history(self):
        ...
        year = self.yy.get()
        if day == 'select day' or month == 'select month' or year == 'select year':
            showerror('Error Fields', 'Choose fields')
        else:
            query_date = f'{day}-{m_val}-{year}'
            ...
            try:
                ...
                for row in rows:
                    ...
                    formatted_date = f'{dd}-{mm}-{yy}'

                    if query_date == formatted_date:
                        url_dict.update({url: extracted_time})

                data_set = sorted(url_dict.items(), key=operator.itemgetter(1))
                self.lbl_disp['fg'] = 'blue'
                self.lbl_disp['text'] = f'Date: {day}-{month}-{year} || Records: {len(data_set)}'
                if len(data_set) > 0:
                    for data in data_set:
                        self.lbox.insert(END, f'Time: {data[1]}     {data[0]}')
                else:
                    self.lbox.insert(END, '')
                    showwarning('Empty', f'There is no record for {query_date}')
